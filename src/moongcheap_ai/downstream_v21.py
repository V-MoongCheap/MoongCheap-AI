...
from .data_foundation.health_v1 import classify_record_type, normalize_form, parse_intake, split_recognition_number
...
def run_downstream_v21(frame: pd.DataFrame, output_dir: Path, count: int = 32) -> dict[str, Any]:
    ...
    report = ["# AI V1 Pipeline Report", "", "## Category", f"- Service Category 후보: {len(taxonomy['categories'])}", f"- Mapping 성공 Product: {int((catalog['mapping_status'] != 'UNMAPPED').sum())}", f"- UNMAPPED Product: {int((catalog['mapping_status'] == 'UNMAPPED').sum())}", "", "## Catalog", f"- Provisional Catalog 후보: {len(catalog)}", f"- Finished Product Candidate: {int((catalog['record_type_candidate'] == 'FINISHED_PRODUCT_CANDIDATE').sum())}", f"- Ingredient Material Candidate: {int((catalog['record_type_candidate'] == 'INGREDIENT_MATERIAL_CANDIDATE').sum())}", f"- Uncertain: {int((catalog['record_type_candidate'] == 'UNCERTAIN').sum())}", "", "## Facet", f"- Category별 결과: {len(summary)}개", f"- 전체 Facet Candidate: {len(review)}", "- Taxonomy status: PROVISIONAL", "", "## Demand", f"- Synthetic Demand: {len(demands)}", f"- Labeling 성공: {int((labeled['label_status'] == 'LABELED').sum())}", f"- Labeling 검토: {int((labeled['label_status'] != 'LABELED').sum())}", f"- unresolved 포함: {int((labeled['unresolved_items'] != '[]').sum())}", "", "## Clustering Input", f"- Export Row: {len(cluster)}", "- 필수 ID 미확정: catalog_id는 의도적으로 공란", "", "## 상태", "- DB 연결 없음", "- Category/Catalog/Facet/Code는 PROVISIONAL 또는 NEEDS_REVIEW", "- Synthetic Demand는 MOCK 데이터", "- Final ID, Alias, Multi-value Encoding은 미확정"]
