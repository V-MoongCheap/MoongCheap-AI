...
from .data_foundation.catalog import (
    build_catalog,
    resolve_identity,
    source_rows_to_staging,
)
...
from .data_foundation.audit import (
    audit_aihub,
    build_category_source_mapping,
    product_catalog_coverage,
    write_today_result,
)
from .data_foundation.facet import (
    preprocess_i0030,
    preprocess_i2710,
    repeated_terms,
        ...
        try:
            ...
            if suffix == ".csv":
                ...
                frame = (
                    ...
                    and {"images", "annotations"}.issubset(value)
                    else pd.DataFrame(
                        value
                        if isinstance(value, list)
                        else value.get("data", value.get("rows", []))
                    )
                )
            else:
                continue
...
def run(root: Path, stage: str = "all") -> None:
    ...
    if not image_root.exists() or not any(
        path.is_file() for path in image_root.rglob("*")
    ):
        report(
            p["reports"] / "aihub_product_image_audit.json",
            {
                "status": "SKIPPED",
                "reason": "AI-Hub product image raw data not found",
                "required_directory": "data/raw/aihub/product_image",
            },
        )
    if stage in {"all", "inspect", "catalog"}:
        if not aihub.exists() or not any(path.is_file() for path in aihub.rglob("*")):
            report(
                p["reports"] / "pipeline_status.json",
                {
                    "status": "SKIPPED",
                    "stages": "01-07",
                    "reason": "AI-Hub raw data not found",
            ...
            report(
                p["reports"] / "aihub_inspection.json",
                {"files": files, "samples": samples},
            )
            pd.DataFrame(files).to_csv(
                p["reports"] / "aihub_file_inventory.csv",
                index=False,
                encoding="utf-8-sig",
            )
            source_frames = [
                source_rows_to_staging(frame, "AI_HUB", path)
                for path, frame in read_frames(aihub)
            ]
            staging = (
            ...
            out = root / "data/interim/products"
            out.mkdir(parents=True, exist_ok=True)
            if not staging.empty:
                staging.to_parquet(out / "product_staging.parquet", index=False)
                staging.to_csv(
                    out / "product_staging_preview.csv",
                    index=False,
                    encoding="utf-8-sig",
                )
                health_mask = (
                    staging["source_category_path"]
                    .fillna("")
                    .astype(str)
                    .str.contains(r"(?:^| > )15_", regex=True, na=False)
                )
                health_subset = staging.loc[health_mask].copy()
                health_subset.to_parquet(
                    out / "aihub_health_food_subset_v0.parquet", index=False
                )
                health_subset.to_csv(
                    out / "aihub_health_food_subset_v0.csv",
                    index=False,
                    encoding="utf-8-sig",
                )
                report(
                    p["reports"] / "aihub_health_food_subset.json",
                    {
                        "status": "COMPLETED",
                        ...
                        "unique_valid_barcode_count": int(
                            health_subset.loc[
                                health_subset["barcode_valid"], "barcode"
                            ].nunique()
                        ),
                        "unique_product_name_count": int(
                            health_subset["product_name_normalized"].nunique()
                        ),
                        "observed_kan_codes": sorted(
                            health_subset["kan_code"]
                            .dropna()
                            .astype(str)
            ...
            audit_conflicts.to_csv(
                p["reports"] / "aihub_barcode_conflicts.csv",
                index=False,
                encoding="utf-8-sig",
            )
            if not conflicts.empty:
                conflicts.to_csv(
                    p["reports"] / "product_identity_conflicts.csv",
                    index=False,
                    encoding="utf-8-sig",
                )
            catalog, provenance = build_catalog(resolved)
            catalog_dir = root / "data/processed/product_catalog"
            catalog_dir.mkdir(parents=True, exist_ok=True)
            catalog.to_parquet(catalog_dir / "product_catalog_v1.parquet", index=False)
            catalog.to_csv(
                catalog_dir / "product_catalog_v1.csv",
                index=False,
                encoding="utf-8-sig",
            )
            health_codes = set(
            ...
            catalog[catalog["kan_code"].astype(str).isin(health_codes)].to_parquet(
                catalog_dir / "product_catalog_health_food_subset_v0.parquet",
                index=False,
            )
            provenance.to_csv(
                catalog_dir / "product_catalog_source.csv",
                index=False,
                encoding="utf-8-sig",
            )
            coverage_summary = product_catalog_coverage(staging, catalog)
            report(p["reports"] / "product_catalog_coverage.json", coverage_summary)
            build_category_source_mapping(staging).to_csv(
                p["processed_category"] / "category_source_mapping.csv",
                index=False,
                encoding="utf-8-sig",
            )
            build_aihub_category_hierarchy(
                staging, p["processed_category"] / "aihub_category_hierarchy_v0.csv"
            )
            ...
            build_observed_kan(
                staging, p["processed_category"] / "category_master_v1.csv"
            )
            report(
                p["reports"] / "pipeline_status.json",
                {
    ...
    if stage in {"all", "mfds", "facet"}:
        ...
        if api_key:
            for service in ("I0030", "I2710"):
                try:
                    ...
                    result = collect(
                        service, api_key, p["raw_mfds"] / service, max_pages=max_pages
                    )
                    report(
                        p["reports"] / f"mfds_{service.lower()}_collection.json", result
                    )
                    ...
                    report(
                        p["reports"] / f"mfds_{service.lower()}_collection.json",
                        {"status": "FAILED", "reason": str(exc)},
                    )
                    LOGGER.warning(
                        "MFDS %s failed; continuing to write stage reports.", service
                    )
                    break
        else:
        ...
        for service in ("I0030", "I2710"):
            collection = read_report(
                p["reports"] / f"mfds_{service.lower()}_collection.json"
            )
            if (
                collection
                and collection.get("status") != "FAILED"
                and collection.get("rows", 0) > 0
            ):
                completed_mfds_services.add(service)
        ...
        if stage in {"all", "mfds", "facet"} and mfds_has_raw:
            i0030 = preprocess_i0030(
                p["raw_mfds"] / "I0030", p["interim_facet"] / "i0030_products_clean.csv"
            )
            i2710 = preprocess_i2710(
                p["raw_mfds"] / "I2710", p["interim_facet"] / "i2710_reference.csv"
            )
            report(p["reports"] / "mfds_i0030_preprocessing.json", i0030)
            report(p["reports"] / "mfds_i2710_preprocessing.json", i2710)
            ...
            source.to_csv(
                p["interim_facet"] / "i0030_products_clean_dedup.csv",
                index=False,
                encoding="utf-8-sig",
            )
            duplicate_rows.to_csv(
                p["interim_facet"] / "i0030_duplicate_review.csv",
                index=False,
                encoding="utf-8-sig",
            )
            ...
            try:
                ...
                category_seed_path = (
                    p["processed_category"] / "health_category_seed_v0.csv"
                )
                category_seed_path.parent.mkdir(parents=True, exist_ok=True)
                category_seed.to_csv(
                    category_seed_path, index=False, encoding="utf-8-sig"
                )
                category_seed_status = {
                    "status": "DRAFT_PENDING_HUMAN_REVIEW",
                    "rows": len(category_seed),
                    "output": str(category_seed_path),
                }
            ...
            report(
                p["reports"] / "health_category_seed_status.json", category_seed_status
            )
            ...
            terms.to_csv(
                p["interim_facet"] / "repeated_terms.csv",
                index=False,
                encoding="utf-8-sig",
            )
            structured_distribution(
                ...
                p["interim_facet"] / "structured_value_distribution.csv",
                index=False,
                encoding="utf-8-sig",
            )
            if completed_mfds_services == {"I0030", "I2710"}:
                ...
                (p["processed_facet"] / "facet_taxonomy_v0.json").write_text(
                    json.dumps(taxonomy, ensure_ascii=False, indent=2), encoding="utf-8"
                )
                facet_status = {
                    "status": "COMPLETED",
                    "i0030_rows": i0030["processed_rows"],
                    "i2710_rows": i2710["processed_rows"],
                }
            else:
                facet_status = {
                    "status": "PARTIAL",
                    "reason": "Only one MFDS service has raw pages; approved taxonomy was not generated.",
                    "i0030_rows": i0030["processed_rows"],
                    "i2710_rows": i2710["processed_rows"],
                }
            ...
            report(p["reports"] / "facet_status.json", facet_status)
    report(
        p["reports"] / "backend_schema_recommendations.json",
        {
            "migration_applied": False,
            "recommendations": [
                "thumbnail_url nullable 또는 placeholder 정책",
                "product_catalog_source에 external_product_id/barcode/source 저장",
                "category_source_mapping에 KAN 매핑 저장",
            ],
        },
    )
    ...
    if (
        not completed_mfds_services
        and collection_report
        and collection_report.get("status") == "FAILED"
    ):
        mfds_status = {
            "status": "FAILED",
            "reason": collection_report.get("reason", "MFDS collection failed"),
        }
        report(p["reports"] / "mfds_status.json", mfds_status)
    else:
    ...
    if stage in {"all", "mfds", "facet"}:
        ...
        if pipeline_report:
            ...
            pipeline_report["health_category_seed_status"] = (
                category_seed_status
                or read_report(p["reports"] / "health_category_seed_status.json")
            )
            pipeline_report["status"] = (
                "COMPLETED"
                if mfds_status.get("status") == "COMPLETED"
                and facet_status.get("status") == "COMPLETED"
                else "COMPLETED_WITH_WARNINGS"
            )
            report(p["reports"] / "pipeline_status.json", pipeline_report)
    write_today_result(
        p["reports"] / "TODAY_RESULT.md",
        aihub_audit_summary,
        coverage_summary,
        mfds_status,
        facet_status,
...
def main() -> None:
    ...
    parser.add_argument(
        "--stage", choices=["all", "inspect", "catalog", "mfds", "facet"], default="all"
    )
