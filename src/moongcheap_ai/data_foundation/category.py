    ...
    if not staging.empty and "kan_code" in staging.columns:
        for code, group in staging.groupby("kan_code", dropna=False, sort=True):
            ...
            if not code:
                continue
            name = next(
                (str(x) for x in group.get("source_category_name", []) if str(x)), ""
            )
            rows.append(
                {
                    "category_seed_id": f"kan-{code}",
                    "category_key": f"KAN:{code}",
                    "kan_code": code,
                    "name": name or code,
                    "parent_category_key": "",
                    "depth": 1,
                    "category_path": name or code,
...
def build_aihub_category_hierarchy(
    staging: pd.DataFrame, output: Path
) -> dict[str, int]:
    """Create an observed hierarchy from preserved AI-Hub source paths."""
    nodes: dict[str, dict] = {}
    if not staging.empty and {"source_category_path", "kan_code"}.issubset(
        staging.columns
    ):
            ...
            for depth, name in enumerate(parts, 1):
                ...
                node = nodes.setdefault(
                    category_path,
                    {
                        ...
                        "parent_category_key": f"AIHUB:{parent_path}"
                        if parent_path
                        else "",
                        "name": name,
                        "depth": depth,
                ...
                node["observed_kan_codes"].update(
                    set(group["kan_code"].astype(str).loc[lambda values: values.ne("")])
                )
    rows = []
