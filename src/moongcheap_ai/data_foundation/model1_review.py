...
FORM_MAP[_u("\ud658")] = "pill"
CATEGORY_NAMES = {
    "health-functional-food:vitamin_mineral": _u(
        "\ube44\ud0c0\ubbfc\u00b7\ubbf8\ub124\ub784"
    ),
    "health-functional-food:probiotics": _u(
        "\uc720\uc0b0\uade0\u00b7\ud504\ub85c\ubc14\uc774\uc624\ud2f1\uc2a4"
    ),
...
REGULATED_CANONICAL_PATTERNS = (
    ...
    (
        re.compile(_u(r"\ud608\uc911\s*\ucf5c\ub808\uc2a4\ud14c\ub864")),
        _u("\ud608\uc911 \ucf5c\ub808\uc2a4\ud14c\ub864"),
    ),
    (re.compile(_u(r"\ud608\ud589")), _u("\ud608\ud589")),
    (re.compile(_u(r"\uc6d4\uacbd\uc804")), _u("\uc6d4\uacbd\uc804 \ubd88\ud3b8")),
    (
        re.compile(_u(r"\uba74\uc5ed\uacfc\ubbfc\ubc18\uc751.*\ud53c\ubd80")),
        _u("\uba74\uc5ed\uacfc\ubbfc\ubc18\uc751 \ud53c\ubd80 \uc0c1\ud0dc"),
    ),
)
RECOGNITION_NUMBER_RE = re.compile(
    r"(?:\uae30\ub2a5\uc131\uc6d0\ub8cc\uc778\uc815\uc81c|\uc778\uc815\uc81c|\uc0dd\ub9ac\ud65c\uc131\uae30\ub2a5\s*)?(\d{4})\s*[-\u2013]\s*(\d+)\s*\ud638?",
    re.IGNORECASE,
)
...
def review_candidates(candidates: pd.DataFrame, inputs: pd.DataFrame) -> pd.DataFrame:
    ...
    normalized_input = (
        input_frame[text_columns].astype(str).map(normalize_text)
        if text_columns
        else pd.DataFrame(index=input_frame.index)
    )
    for _, candidate in candidates.fillna("").iterrows():
        raw_name = str(candidate.get("name", ""))
        facet_id, scope = FACET_MAP.get(
            normalize_text(raw_name),
            (normalize_text(raw_name).replace(" ", "_"), "UNKNOWN"),
        )
        raw_value = str(candidate.get("value", ""))
        normalized = normalize_value(facet_id, raw_value)
        category = str(candidate.get("category_key", ""))
        category_rows = (
            input_frame[input_frame["category_key"].astype(str).eq(category)]
            if "category_key" in input_frame
            else input_frame.iloc[0:0]
        )
        category_normalized = normalized_input.loc[category_rows.index]
        needle = normalize_text(raw_value)
        matching_rows = (
            category_rows[
                category_normalized.map(
                    lambda value, search=needle: search in value if search else False
                ).any(axis=1)
            ]
            if text_columns and not category_rows.empty
            else category_rows.iloc[0:0]
        )
        evidence_matches = _contains(
            str(candidate.get("source_text", "")), normalize_text(raw_value)
        )
        reasons: list[str] = []
        status = "ACCEPT_CANDIDATE"
        if scope == "UNKNOWN":
            status, reasons = "REVIEW_REQUIRED", ["unrecognized_facet"]
        if not normalized:
            status, reasons = "REJECT", reasons + ["empty_value"]
        if any(token in normalize_text(raw_value) for token in OUT_OF_SCOPE):
            status, reasons = (
                "REJECT",
                reasons + ["out_of_scope_product_or_treatment_term"],
            )
        if any(marker in raw_value for marker in RAW_SENTENCE_MARKERS):
            status, reasons = (
                ("REVIEW_REQUIRED" if status != "REJECT" else status),
                reasons + ["raw_natural_language_sentence"],
            )
        if scope == "EVIDENCE_ONLY":
            status, reasons = (
                ("REVIEW_REQUIRED" if status != "REJECT" else status),
                reasons + ["regulated_function_is_evidence_only"],
            )
        if scope in {"DEMAND", "DEMAND_SIGNAL"}:
            status, reasons = (
                ("REVIEW_REQUIRED" if status != "REJECT" else status),
                reasons + ["demand_signal_not_product_facet"],
            )
        if facet_id == "intake_method":
            status, reasons = (
                ("REVIEW_REQUIRED" if status != "REJECT" else status),
                reasons + ["requires_structured_intake_parser"],
            )
        if not evidence_matches:
            status, reasons = (
                ("REVIEW_REQUIRED" if status != "REJECT" else status),
                reasons + ["evidence_text_mismatch"],
            )
        if len(matching_rows) == 0:
            status, reasons = "REJECT", reasons + ["no_matching_input_row"]
        elif len(matching_rows) < 2 and status == "ACCEPT_CANDIDATE":
            status, reasons = "REVIEW_REQUIRED", reasons + ["single_observed_input_row"]
        rows.append(
            {
                **candidate.to_dict(),
...
def normalize_review_candidates(reviewed: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in reviewed.fillna("").iterrows():
        ...
        atoms = (
            ...
            if facet_id in {"functional_ingredients", "regulated_function"}
            else [value]
        )
        if facet_id == "regulated_function":
            atoms = [re.sub(r"^\s*[①-⑳\d.)]+\s*", "", part).strip() for part in atoms]
        ...
        if facet_id == "intake_method":
            ...
            if intake_match:
                intake_days, intake_frequency = (
                    intake_match.group(1) or "",
                    intake_match.group(2) or "",
                )
        price_min = price_max = ""
        if facet_id == "price_band":
            ...
            if price_match:
                price_min, price_max = (
                    (price_match.group(3) or price_match.group(2) or "0"),
                    (price_match.group(1) or price_match.group(4) or ""),
                )
        for atom in atoms:
            rows.append(
                {
                    **row.to_dict(),
                    ...
                    "price_min": price_min,
                    "price_max": price_max,
                    "normalization_status": "PARSED" if atom else "EMPTY",
                    "finalization_status": "EVIDENCE_ONLY"
                    if facet_id == "regulated_function"
                    else str(row.get("review_status", "REVIEW_REQUIRED")),
                }
            )
    return pd.DataFrame(rows)
...
def collapse_same_model_candidates(normalized: pd.DataFrame) -> pd.DataFrame:
    ...
    rows: list[dict[str, Any]] = []
    for _, group in normalized.fillna("").groupby(
        ...
        product_ids = sorted(
            {
                str(value)
                for value in group["source_product_id"].astype(str)
                if str(value)
            }
        )
        for column in (
            ...
            values = (
                sorted(
                    {str(value) for value in group[column].astype(str) if str(value)}
                )
                if column in group
                else []
            )
...
def build_review_queue(normalized: pd.DataFrame) -> pd.DataFrame:
    ...
    for column in ("human_decision", "human_value", "human_note"):
        queue[column] = ""
    return (
        queue.reindex(columns=columns)
        .drop_duplicates("review_id")
        .sort_values(
            ["category_key", "facet_id", "facet_value", "model"], ignore_index=True
        )
    )
...
def apply_human_decisions(queue: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    result["human_decision"] = (
        result["human_decision"].astype(str).str.strip().str.upper()
    )
    invalid = sorted(
        set(result.loc[~result.human_decision.isin(allowed), "human_decision"])
    )
    if invalid:
        raise ValueError(
            f"Unsupported human_decision values: {invalid}; use ACCEPT, REJECT, or DEFER"
        )
    result["resolved_value"] = result["human_value"].where(
        result["human_value"].ne(""), result["facet_value"]
    )
    result["resolution_status"] = result["human_decision"].map(
        {
...
def write_review_artifacts(
    candidates_path: Path, input_path: Path, output_dir: Path
) -> dict[str, Any]:
    output_dir.mkdir(parents=True, exist_ok=True)
    reviewed = review_candidates(
        pd.read_csv(candidates_path).fillna(""),
        pd.read_json(input_path, lines=True).fillna(""),
    )
    reviewed.to_csv(
        output_dir / "multisource_candidate_review_v1.csv",
        index=False,
        encoding="utf-8-sig",
    )
    summary = (
        reviewed.groupby(["review_status", "review_scope"], dropna=False)
        .size()
        .reset_index(name="rows")
        if not reviewed.empty
        else pd.DataFrame(columns=["review_status", "review_scope", "rows"])
    )
    summary.to_csv(
        output_dir / "multisource_candidate_review_summary_v1.csv",
        index=False,
        encoding="utf-8-sig",
    )
    lines = [
    ...
    for status, count in (
        reviewed["review_status"].value_counts().items() if not reviewed.empty else []
    ):
    ...
    lines += [
        ...
        "## 기준",
        "",
        "- 같은 Category의 입력 행에 후보 값이 실제로 존재하는지 확인",
        "- 증거 원문과 후보 값의 일치 여부 확인",
        "- 규제 기능은 근거로만 보존",
        "- 가격대·구매 의도는 Demand 신호로 분리",
        "- 범위 이탈 표현은 제외",
        "- 섭취 방법은 구조화 파서 검토 대상으로 분류",
    ]
    ...
    return {
        "rows": len(reviewed),
        "status_counts": reviewed["review_status"].value_counts().to_dict()
        if not reviewed.empty
        else {},
    }
