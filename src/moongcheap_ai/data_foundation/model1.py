...
def _build_compact_prompt(category: str, products: list[dict[str, Any]], prompt_version: str) -> str:
    """Build a bounded prompt for small-context Korean models."""
    compact_rows = []
    keep = ("source_product_id", "source_type", "product_form", "functional_ingredients", "intake_method", "evidence_text")
...
class OllamaAdapter:
    ...
    def generate_facet_candidates(self, category: str, products: list[dict[str, Any]], prompt_version: str) -> dict[str, Any]:
        ...
        prompt = _build_compact_prompt(category, products, prompt_version) if compact else _build_prompt(self.prompt_path, category, products, prompt_version)
...
class TransformersAdapter:
    ...
    def __init__(self, model: str, prompt_path: Path | None = None, max_new_tokens: int | None = None) -> None:
        self.model = model
        self.prompt_path = prompt_path or PROMPT_PATH
        self.max_new_tokens = max_new_tokens or int(os.getenv("MODEL1_MAX_NEW_TOKENS", "1024"))
        self.use_chat_template = os.getenv("MODEL1_USE_CHAT_TEMPLATE", "true").casefold() == "true"
        self.trust_remote_code = os.getenv("MODEL1_TRUST_REMOTE_CODE", "false").casefold() == "true"
        self._pipeline = None
    ...
    def _generated_text(output: Any) -> str:
        generated = output[0].get("generated_text", "") if isinstance(output, list) and output else ""
...
def sample_products(frame: pd.DataFrame, max_per_category: int = 24, seed: int = 42) -> pd.DataFrame:
    ...
    for column in ("source_product_id", "product_type", "source_category_path", "product_form", "functional_ingredients"):
    ...
    try:
        ...
        for facet_index, facet in enumerate(payload["facets"]):
            if not isinstance(facet, dict):
                failures.append({"failure_type": "INVALID_FACET", "detail": str(facet_index)})
                continue
            name = str(facet.get("name") or facet.get("facet_name") or facet.get("facet_key", "")).strip()
            if not name:
            ...
            if not values and facet.get("value") not in (None, ""):
                values = [{"value": facet.get("value", ""), "aliases": facet.get("aliases", [])}]
            if not values:
                failures.append({"failure_type": "EMPTY_VALUE", "detail": name}); continue
            valid_values = []
            for value_index, value in enumerate(values):
                if not isinstance(value, dict):
                    failures.append({"failure_type": "INVALID_VALUE", "detail": f"{name}[{value_index}]"})
                    continue
            ...
            for evidence_index, item in enumerate(evidence):
                if not isinstance(item, dict):
                    failures.append({"failure_type": "INVALID_EVIDENCE", "detail": f"{name}[{evidence_index}]"})
                    continue
                source_id = str(item.get("source_product_id") or item.get("product_id") or "").strip()
                source_text = str(item.get("source_text") or item.get("evidence_text") or "").strip()
                source_field = str(item.get("source_field") or "").strip()
                if source_id not in valid_ids:
                    failures.append({"failure_type": "HALLUCINATED_EVIDENCE", "detail": source_id}); continue
                if not source_text:
                    failures.append({"failure_type": "EVIDENCE_TEXT_MISSING", "detail": source_id})
                    continue
                if source_field not in input_products.columns:
                    source_field = ""
                if not source_field:
                    source_row = input_products.loc[input_products["source_product_id"].astype(str) == source_id].iloc[0]
                    source_field = next((column for column in input_products.columns if source_text in str(source_row.get(column, ""))), "")
                if not source_field:
                    failures.append({"failure_type": "EVIDENCE_FIELD_UNRESOLVED", "detail": source_id}); continue
                ...
                for value, value_text in valid_values:
                    ...
                    if not isinstance(aliases, list):
                        aliases = [aliases]
                    rows.append({"category_key": category_key, "category_name": category_name, "facet_id_candidate": facet.get("facet_id_candidate", ""), "name": name, "definition": facet.get("definition", ""), "value": value_text, "alias": "|".join(str(alias).strip() for alias in aliases if str(alias).strip()), "source_product_id": source_id, "source_field": source_field, "source_text": source_text, "status": "PROVISIONAL_MODEL_OUTPUT"})
