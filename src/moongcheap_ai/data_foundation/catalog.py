    ...
    try:
        start = (
            next(
                ...
                if part.lower() == "logistics_product"
            )
            + 1
        )
...
def source_rows_to_staging(
    frame: pd.DataFrame, source: str, source_file: str
) -> pd.DataFrame:
    columns = [str(c) for c in frame.columns]
    selected = {
        field: pick_column(columns, candidates) for field, candidates in ALIASES.items()
    }
    rows = []
    file_category_path = source_category_path_from_file(source_file)
    for index, row in frame.iterrows():
        name = (
            str(row.get(selected["product_name_raw"], "") or "")
            if selected["product_name_raw"]
            else ""
        )
        barcode_raw = (
            str(row.get(selected["barcode"], "") or "") if selected["barcode"] else ""
        )
        rows.append(
            {
                "source": source,
                "source_product_id": barcode_raw,
                "barcode": normalize_barcode(barcode_raw),
                "barcode_valid": barcode_valid(normalize_barcode(barcode_raw)),
                "product_name_raw": name,
                "product_name_normalized": normalize_name(name),
                "manufacturer": str(row.get(selected["manufacturer"], "") or "")
                if selected["manufacturer"]
                else "",
                "brand": str(row.get(selected["brand"], "") or "")
                if selected["brand"]
                else "",
                "model_code": "",
                "quantity_text": "",
                ...
                "source_category_id": str(
                    row.get(selected["source_category_id"], "") or ""
                )
                if selected["source_category_id"]
                else "",
                "source_category_name": str(
                    row.get(selected["source_category_name"], "") or ""
                )
                if selected["source_category_name"]
                else "",
                "source_category_path": file_category_path
                or (
                    str(row.get(selected["source_category_path"], "") or "")
                    if selected["source_category_path"]
                    else ""
                ),
                "kan_code": str(row.get(selected["kan_code"], "") or "")
                if selected["kan_code"]
                else "",
                "description_raw": "",
                "spec_raw": "",
                "image_url": str(row.get(selected["image_url"], "") or "")
                ...
                "fragile": str(row.get("fragile", "") or "")
                if "fragile" in frame.columns
                else "",
                "refrigerate": str(row.get("refrigerate", "") or "")
                if "refrigerate" in frame.columns
                else "",
            }
        )
...
def resolve_identity(staging: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for indexes in barcode_groups.values():
        ...
        kans = {
            str(frame.loc[i, "kan_code"]) for i in indexes if frame.loc[i, "kan_code"]
        }
        seed = f"catalog-{next_id:06d}"
        next_id += 1
        for i in indexes:
            groups[i] = seed
        if len(names) > 1 or len(kans) > 1:
            conflicts.append(
                {
                    "barcode": str(frame.loc[indexes[0], "barcode"]),
                    "product_names": "|".join(sorted(names)),
                    "kan_codes": "|".join(sorted(kans)),
                    "conflict_type": "same_barcode_different_metadata",
                }
            )
    for index in frame.index:
        if index not in groups:
            groups[index] = f"catalog-{next_id:06d}"
            next_id += 1
...
def build_catalog(resolved: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for catalog_id, group in resolved.groupby("catalog_seed_id", sort=True):
        ...
        rows.append(
            {
                "catalog_seed_id": catalog_id,
                "name": next((x for x in group["product_name_normalized"] if x), ""),
                "category_key": first.get("source_category_path", ""),
                "kan_code": next((x for x in group["kan_code"] if x), ""),
                "barcode": next((x for x in group["barcode"] if x), ""),
                "manufacturer": next((x for x in group["manufacturer"] if x), ""),
                "brand": next((x for x in group["brand"] if x), ""),
                "model_code": "",
                "capacity_text": "",
                "package_text": "",
                "spec_summary": "",
                "description": "",
                "thumbnail_url": next((x for x in group["image_url"] if x), ""),
                "list_price": "",
                "status": "ACTIVE",
        ...
        provenance.extend(
            ...
            .assign(catalog_seed_id=catalog_id)
            .to_dict("records")
        )
