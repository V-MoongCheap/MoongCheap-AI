...
def build_analysis(
    input_path: Path, evidence_dir: Path, report_path: Path
) -> dict[str, object]:
    ...
    for limit in (100, 200, 300, 400, 500):
        ...
        keys = (
            set(
                zip(
                    subset_evidence["normalized_attribute"],
                    subset_evidence["normalized_value"],
                )
            )
            if not subset_evidence.empty
            else set()
        )
        previous = (
            milestones[-1]["cumulative_unique_facet_candidates"] if milestones else 0
        )
        milestones.append(
            {
                "reviews": min(limit, len(rows)),
                "cumulative_unique_facet_candidates": len(keys),
                "new_facet_candidates": max(0, len(keys) - previous),
            }
        )
        if limit >= len(rows):
            break
    ...
    report = {
        ...
        "usable_review_text_count": sum(
            bool(str(row.get("review_text") or "").strip()) for row in rows
        ),
        "duplicate_rate": 1
        - len({str(row.get("source_review_id")) for row in rows}) / len(rows)
        if rows
        else 0,
        "product_mapping_rate": len(mapped) / len(rows) if rows else 0,
        "hff_confirmed_review_count": len(mapped),
        "rating_coverage": sum(row.get("rating") is not None for row in rows)
        / len(rows)
        if rows
        else 0,
        "date_coverage": sum(
            bool(str(row.get("review_date") or "").strip()) for row in rows
        )
        / len(rows)
        if rows
        else 0,
        "unique_product_count": len(product_counts),
        "top_10_product_review_share": sum(
            item["review_count"] for item in top_products
        )
        / len(rows)
        if rows
        else 0,
    ...
    evidence.to_csv(
        evidence_dir / "nutrime_review_evidence_preview.csv",
        index=False,
        encoding="utf-8-sig",
    )
    (evidence_dir / "nutrime_review_analysis.json").write_text(
    ...
    lines.extend(
        f"| {item['reviews']} | {item['cumulative_unique_facet_candidates']} | {item['new_facet_candidates']} |"
        for item in milestones
    )
    lines += [
        "",
        "## Top Product Concentration",
        "| source_product_id | review_count | share |",
        "|---|---:|---:|",
    ]
    lines.extend(
        f"| {item['source_product_id']} | {item['review_count']} | {item['share']:.2%} |"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input",
        type=Path,
        default=Path("data/raw/reviews/nutrime/nutrime_reviews_500_20260909.jsonl"),
    )
    parser.add_argument(
        "--evidence-dir", type=Path, default=Path("data/processed/reviews/nutrime")
    )
    parser.add_argument(
        "--report", type=Path, default=Path("reports/review_facet_saturation.md")
    )
    args = parser.parse_args()
    print(
        json.dumps(
            build_analysis(args.input, args.evidence_dir, args.report),
            ensure_ascii=False,
        )
    )
