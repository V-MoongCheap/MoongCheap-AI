...
PRODUCT_CONTEXT = re.compile(
    r"브랜드|상품명|제품명|모델명|상품번호|\b[A-Z]{2,}\b|\d{2,}"
)
GENERIC_EXPRESSIONS = {
    "맛이",
    "맛은",
...
def _risk_flags(expression: str, examples: list[str]) -> list[str]:
    ...
    if len(expression.replace(" ", "")) <= 4 or expression in {
        "향이",
        "맛이",
        "맛은",
        "포장이",
...
def build_sheet(
    candidates: pd.DataFrame, raw: pd.DataFrame
) -> tuple[pd.DataFrame, dict[str, int]]:
    rows: list[dict[str, object]] = []
    conflict_groups = candidates.groupby(
        candidates["candidate_expression"].astype(str).str.strip()
    )["facet_id"].nunique()
    for item in candidates.itertuples(index=False):
        expression = str(item.candidate_expression).strip()
        matches = raw[
            raw["review_text"].str.contains(re.escape(expression), regex=True, na=False)
        ]
        ...
        conflict = (
            ...
            if int(conflict_groups.get(expression, 0)) > 1
            else ""
        )
        ...
        rows.append(
            {
                ...
                "canonical_facet_name": CANONICAL_NAMES.get(
                    str(item.facet_id), str(item.facet_id)
                ),
                ...
                if conflict or "MEDICAL_OUTCOME" in flags
                else "NORMAL",
            }
        )
    sheet = pd.DataFrame(rows)
    sheet = sheet.sort_values(
        ["review_priority", "occurrence_count", "semantic_similarity"],
    ...
    stats = {
        ...
        "review_supported_combination_count": int(
            candidates[["facet_id", "value_candidate"]].drop_duplicates().shape[0]
        ),
        "category_facet_combination_count": 0,
        "cross_facet_conflict_count": int(sheet["conflict_status"].ne("").sum()),
        "generic_risk_count": int(
            sheet["risk_flags"].str.contains("TOO_GENERIC", na=False).sum()
        ),
        "medical_risk_count": int(
            sheet["risk_flags"].str.contains("MEDICAL_OUTCOME", na=False).sum()
        ),
        "needs_review_count": int(
            sheet["reviewer_decision"].eq("PENDING_REVIEW").sum()
        ),
    }
    return sheet, stats
...
def build_report(
    sheet: pd.DataFrame, stats: dict[str, int], output: Path, input_path: Path
) -> None:
    lines = [
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", type=Path, default=Path("data/review/model1_alias_human_review.csv")
    )
    parser.add_argument(
        "--report",
        type=Path,
        default=Path("reports/model1_alias_human_review_guide.md"),
    )
    args = parser.parse_args()
    candidates = pd.read_csv(args.input, dtype=str).fillna("")
    candidates["occurrence_count"] = (
        pd.to_numeric(candidates["occurrence_count"], errors="coerce")
        .fillna(0)
        .astype(int)
    )
    candidates["semantic_similarity"] = pd.to_numeric(
        candidates["semantic_similarity"], errors="coerce"
    ).fillna(0.0)
