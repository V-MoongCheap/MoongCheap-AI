...
def build_value_crosswalk(taxonomy: dict[str, Any]) -> dict[str, Any]:
    ...
    for category in taxonomy["categories"]:
        ...
        facet = next(
            (f for f in category["facets"] if f["name"] == "product_form"), None
        )
        local: dict[str, Any] = {}
        if facet:
            values = {
                normalize(v.get("value")): v
                for v in facet.get("values", [])
                if int(v.get("code", 0)) != 0
            }
...
def _accepted(row: pd.Series) -> bool:
    ...
    return decision == "APPROVE_ALIAS" or (
        decision == "NEEDS_REVIEW" and bool(corrected)
    )


def build_alias_artifacts(
    review: pd.DataFrame, taxonomy: dict[str, Any], crosswalk: dict[str, Any]
    ...
    for _, row in review.iterrows():
        ...
        if accepted:
            if facet not in taxonomy_facets:
                ...
                status = "DEFERRED_TAXONOMY_NOT_APPROVED"
                reason = (
                    "Facet/value target is not approved for V2.2 alias application."
                )
        if status == "APPLIED":
            key = (facet, target)
            group = registry_groups.setdefault(
                key,
                {
                    "facet_name": facet,
                    "canonical_value": target,
                    "surfaces": [],
                    "category_local_values": {},
                },
            )
        ...
        audit.append(
            {
                ...
                "category_local_codes": json.dumps(
                    {k: v.get("code") for k, v in category_code_map.items()},
                    ensure_ascii=False,
                    sort_keys=True,
                ),
                "apply_status": status,
    ...
    registry = {
        ...
        "aliases": [
            {
                "facet_name": g["facet_name"],
                "canonical_value": g["canonical_value"],
                "surfaces": g["surfaces"],
                "category_local_values": g["category_local_values"],
            }
        ...
        "summary": {
            ...
            "deferred_rows": int(
                (audit_frame["apply_status"] == "DEFERRED_TAXONOMY_NOT_APPROVED").sum()
            ),
            "rejected_rows": int(
                (audit_frame["apply_status"] == "REJECTED_BY_HUMAN").sum()
            ),
            "target_not_found_rows": int(
                (audit_frame["apply_status"] == "TARGET_NOT_FOUND").sum()
            ),
            "corrected_target_rows": int(
                (
                    (audit_frame["corrected_value_candidate"] != "")
                    & (audit_frame["apply_status"] == "APPLIED")
                ).sum()
            ),
        },
    }
...
def build_taxonomy_v22(source: dict[str, Any]) -> dict[str, Any]:
    ...
    output["taxonomy_policy"] = (
        "Stable MVP taxonomy; category-local facet/value codes preserved from V2.1."
    )
...
def validate_taxonomy(before: dict[str, Any], after: dict[str, Any]) -> dict[str, int]:
    before_codes = {
        (c["category_id"], f["name"], int(v["code"]))
    ...
    duplicate_codes = sum(
        len({int(v["code"]) for v in f["values"]}) != len(f["values"])
        for c in after["categories"]
        for f in c["facets"]
    )
    return {
        "preserved_codes": int(before_codes <= after_codes),
        "code_count_before": len(before_codes),
        "code_count_after": len(after_codes),
        "duplicate_code_facets": duplicate_codes,
    }
        ...
        rows.append(
            {
                ...
                "facet_text": json.dumps(
                    category["facets"], ensure_ascii=False, separators=(",", ":")
                ),
            }
        )
...
def write_report(
    ...
    value_count = sum(
        len(f["values"]) for c in after["categories"] for f in c["facets"]
    )
    lines = [
        ...
        "",
        "## 구조",
        f"- Category: {len(after['categories'])}",
        f"- Facet: {facet_count} (V2.1: {sum(len(c['facets']) for c in before['categories'])})",
        f"- Value: {value_count} (V2.1: {sum(len(f['values']) for c in before['categories'] for f in c['facets'])})",
        "- 신규 전역 Facet: 0",
        "",
    ...
    lines += [
        "",
        "## Alias 적용",
        f"- 전체 검수 Row: {len(audit)}",
        f"- APPLIED: {registry['summary']['applied_rows']}",
        f"- DEFERRED_TAXONOMY_NOT_APPROVED: {registry['summary']['deferred_rows']}",
        f"- REJECTED_BY_HUMAN: {registry['summary']['rejected_rows']}",
        f"- TARGET_NOT_FOUND: {registry['summary']['target_not_found_rows']}",
        f"- corrected_value_candidate 적용 성공 Row: {registry['summary']['corrected_target_rows']}",
        "- Product Form Alias는 Category별 local code로 해석한다.",
        "",
        "## 호환성 검증",
        f"- 기존 Code 보존: {'PASS' if validation['preserved_codes'] else 'FAIL'}",
        f"- Code 수 V2.1 → V2.2: {validation['code_count_before']} → {validation['code_count_after']}",
        f"- 중복 Code Facet 수: {validation['duplicate_code_facets']}",
    ]
    if regression:
        lines += [
            "",
            ...
            f"- V2.1 Rule-only diagnostic agreement: {regression.get('v21_agreement')}",
            f"- V2.2 Rule/Alias-only diagnostic agreement: {regression.get('v22_agreement')}",
            f"- Alias hit: {regression.get('alias_hit')}",
            f"- Corrected alias hit: {regression.get('corrected_alias_hit')}",
...
def main() -> int:
    ...
    parser.add_argument(
        "--alias-review",
        type=Path,
        default=Path("data/review/model1_alias_human_review.csv"),
    )
    parser.add_argument(
        "--new-facet-review",
        type=Path,
    ...
    (config / "facet_taxonomy_v2_2.json").write_text(
        json.dumps(taxonomy_after, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (config / "taxonomy_value_crosswalk_v2.json").write_text(
        json.dumps(crosswalk, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (config / "model1_aliases_reviewed_v2.json").write_text(
        json.dumps(registry, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    data_out = out / "data/processed/downstream_v2_2"
    data_out.mkdir(parents=True, exist_ok=True)
    audit.to_csv(
        data_out / "model1_alias_apply_audit_v2.csv", index=False, encoding="utf-8-sig"
    )
    build_preview(taxonomy_after).to_csv(
        data_out / "category_facet_text_preview.csv", index=False, encoding="utf-8-sig"
    )
    reports = out / "reports"
    reports.mkdir(parents=True, exist_ok=True)
    regression = None
    regression_csv = (
        out / "data/processed/downstream_v2_2/taxonomy_v2_2_demand_regression.csv"
    )
    ...
    write_report(
        reports / "facet_taxonomy_v2_2_decision_report.md",
        taxonomy_before,
        taxonomy_after,
        new_facets,
        registry,
    ...
    for category in taxonomy_after["categories"]:
        change_lines.append(
            f"| `{category['category_id']}` | {len(category['facets'])} | 0 | 0 | 0 | 없음 |"
        )
    change_lines += [
        "",
    ...
    print(
        json.dumps(
            {
                "categories": len(taxonomy_after["categories"]),
                "facets": sum(len(c["facets"]) for c in taxonomy_after["categories"]),
                "values": validation["code_count_after"],
                "alias_summary": registry["summary"],
