...
def add_query_evidence(
    sampled: pd.DataFrame, translated: pd.DataFrame, limit: int
) -> pd.DataFrame:
    if translated.empty or limit <= 0:
        return sampled
    rows = []
    for category_key, category_name in (
        sampled[["category_key", "category_name"]]
        .drop_duplicates()
    ...
    return (
        pd.concat([sampled, pd.DataFrame(rows)], ignore_index=True) if rows else sampled
    )


def run_model(
    model_name: str, sampled: pd.DataFrame, categories: set[str], retries: int
    ...
    for category_key, group in sampled[sampled.category_key.isin(categories)].groupby(
        ...
        for attempt in range(retries + 1):
            try:
                response = adapter.generate_facet_candidates(
                    ...
                    "facet_discovery_multi_model_v1",
                )
                calls += 1
                raw.append(
                    {
        ...
        failures.extend(
            [
                {**item, "model": model_name, "category_key": category_key}
        ...
        grouped.append(
            {
                **best,
                ...
                if model_support >= 2
                else "MODEL_SINGLETON_REVIEW",
            }
        )
    result = pd.DataFrame(grouped).sort_values(
        ["category_key", "selection_score", "facet_norm", "value_norm"],
        ascending=[True, False, True, True],
    )
    return result.drop(
        columns=["facet_norm", "value_norm"], errors="ignore"
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-dir", type=Path, default=Path("data/processed/model1_multi_model_v1")
    )
    ...
    translated = (
        pd.read_parquet(args.translated).fillna("")
        if args.translated.exists()
        else pd.DataFrame()
    )
    sampled = add_query_evidence(sampled, translated, args.query_evidence_per_category)
    sampled.to_json(
        args.output_dir / "model_input_with_kuaisearch_v1.jsonl",
        orient="records",
        lines=True,
        force_ascii=False,
    )
    ...
    candidate_frame = pd.DataFrame(
        all_candidates, columns=[*MODEL_OUTPUT_COLUMNS, "model"]
    )
    ...
    selected.to_csv(
        args.output_dir / "selected_facet_candidates_v1.csv",
        index=False,
        encoding="utf-8-sig",
    )
    ...
    report = {
        ...
        "input_rows": len(sampled),
        "query_evidence_rows": int(
            sampled.source_product_id.astype(str).str.startswith("kuaisearch:").sum()
        ),
        ...
        "failure_rows": len(failures),
        "selection_policy": "model support first, then grounded evidence count; singleton candidates remain review-only",
        "taxonomy_changed": False,
    }
    (args.output_dir / "multi_model_selection_report_v1.json").write_text(
    ...
    for item in reports:
        lines.append(
            f"| {item['model']} | {item['calls']} | {item['candidate_rows']} | {item['successful_categories']} | {item['failure_rows']} | {item['runtime_seconds']} |"
        )
    lines += [
        "",
        "## 자동 선정 규칙",
    ...
    consensus = (
        selected[selected.selection_status.eq("SELECTED_CANDIDATE")]
        if not selected.empty
        else selected
    )
    for row in consensus.head(100).itertuples():
        lines.append(
            f"| {row.category_key} | {row.name} | {row.value} | {row.model_support} | {row.evidence_count} | {row.selection_status} |"
        )
    lines += [
        "",
        "## 주의사항",
