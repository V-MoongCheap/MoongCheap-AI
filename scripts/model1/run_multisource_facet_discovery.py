...
def load_products(path: Path, max_per_category: int = 24) -> pd.DataFrame:
    ...
    sampled = sampled.rename(
        columns={"name": "product_name", "product_type": "source_category"}
    )
    sampled["source_type"] = "MFDS_PRODUCT"
    sampled["evidence_text"] = sampled.apply(
        lambda row: " | ".join(
...
def load_seller_offers(path: Path, max_per_category: int = 24) -> pd.DataFrame:
    ...
    sampled["source_product_id"] = sampled["source_product_id"].map(
        lambda value: value if str(value).startswith("seller:") else f"seller:{value}"
    )
    sampled["evidence_text"] = sampled.apply(
        lambda row: " | ".join(
            _text(row.get(column))
...
def load_demands(path: Path) -> pd.DataFrame:
    ...
    return _as_source_rows(
        pd.concat(rows, ignore_index=True) if rows else normalized,
        "GROUNDED_DEMAND_SYNTHETIC",
    )
...
def load_boards(path: Path) -> pd.DataFrame:
    ...
    return (
        _as_source_rows(pd.DataFrame(rows), "DEMAND_BOARD_SYNTHETIC")
        if rows
        else _empty_sources()
    )
...
def load_translated_queries(path: Path, max_per_category: int = 12) -> pd.DataFrame:
    ...
    for category_key, hints in {
        ...
        mask = query.str.contains(
            "|".join(re.escape(item) for item in hints),
            case=False,
            regex=True,
            na=False,
        )
        for item in (
            frame[mask]
            .drop_duplicates("query_translated")
    ...
    return (
        _as_source_rows(pd.DataFrame(rows), "CONSUMER_SEARCH")
        if rows
        else _empty_sources()
    )
...
def build_multisource_input(
    ...
    frames = [
        _call_sampled_loader(
            load_products, paths["products"], max_products_per_category
        ),
        _call_sampled_loader(
            load_seller_offers, paths["sellers"], max_sellers_per_category
        ),
        _call_sampled_loader(
    ...
    if data.empty:
        return _empty_sources()
    return (
        data[data["category_key"].astype(str).str.startswith("health-functional-food:")]
        .drop_duplicates(["category_key", "source_product_id"])
        .reset_index(drop=True)
    )


def parse_reasoned_output(
    payload: dict[str, Any], input_frame: pd.DataFrame
    ...
    reasons = {
        str(item.get("name", "")): _text(
            item.get("selection_reason") or item.get("reason")
        )
        for item in facets
    }
    value_reasons = {
        (str(item.get("name", "")), str(value.get("value", ""))): _text(
            value.get("value_reason")
        )
        for item in facets
    ...
    source_types = dict(
        zip(
            input_frame["source_product_id"].astype(str),
            input_frame["source_type"].astype(str),
        )
    )
    parsed["selection_reason"] = parsed["name"].map(reasons).fillna("")
    parsed["value_reason"] = [
        value_reasons.get((str(name), str(value)), "")
        for name, value in zip(parsed["name"], parsed["value"])
    ]
    parsed["reason"] = parsed["selection_reason"]
    parsed["evidence_source_type"] = (
        parsed["source_product_id"].map(source_types).fillna("")
    )
    parsed["reason_status"] = parsed["selection_reason"].map(
        lambda value: "PRESENT" if value else "MISSING"
    )
    return parsed, failures


def add_data_selection_reason(
    ...
    source_counts = (
        input_data.groupby(["category_key", "source_type"])["source_product_id"]
        .nunique()
        .to_dict()
    )
    category_counts = (
        input_data.groupby("category_key")["source_product_id"].nunique().to_dict()
    )
    ...
    for key, group in result.groupby(["category_key", "value"], dropna=False):
        ...
        mask = (
            category_rows[text_columns]
            .astype(str)
            .apply(
                lambda column, needle=value_norm: column.map(_normalize).str.contains(
                    re.escape(needle), regex=True, na=False
                )
            )
            .any(axis=1)
            if value_norm
            else pd.Series(False, index=category_rows.index)
        )
    ...
    evidence_counts = (
        result.groupby(["model", "category_key", "name", "value"], dropna=False)[
            "source_product_id"
        ]
        .nunique()
        .to_dict()
    ...
    for row in result.itertuples():
        ...
        evidence_count = int(
            evidence_counts.get((row.model, row.category_key, row.name, row.value), 0)
        )
        source_text = ", ".join(
            f"{item}: {int(source_counts.get((row.category_key, item), 0))}건"
            for item in types
        )
        reasons.append(
            f"{row.model}의 {row.category_key} 후보. 입력 {int(category_counts.get(row.category_key, 0))}건 중 값과 일치하는 행 {count}건; 관찰 출처 {source_text or evidence_type or '없음'}; 모델 근거 {evidence_count}건."
        )
...
def select_candidates(candidates: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in data.groupby(
        ["category_key", "facet_norm", "value_norm"], dropna=False
    ):
        ...
        rows.append(
            {
                **best,
                ...
                "source_type_count": int(sources),
                "selection_score": round(models * 3 + min(int(sources), 5) * 0.5, 3),
                "selection_status": "SELECTED_CANDIDATE"
                if models >= 2 and sources >= 2
                else "REVIEW_ONLY",
            }
        )
    return (
        pd.DataFrame(rows)
        .drop(columns=["facet_norm", "value_norm"], errors="ignore")
        .reset_index(drop=True)
    )
...
def run_model(
    ...
    calls = 0
    batch_count = 0
    for category_key, group in data[data["category_key"].isin(categories)].groupby(
        "category_key", sort=True
    ):
        for batch_start in range(0, len(group), max(1, batch_size)):
            batch_count += 1
            model_group = group.iloc[
                batch_start : batch_start + max(1, batch_size)
            ].copy()
            ...
            for attempt in range(retries + 1):
                try:
                    response = adapter.generate_facet_candidates(
                        category_key,
                        model_group.to_dict("records"),
                        "facet_discovery_multisource_v1",
                    )
                    calls += 1
                    raw.append(
                        {
                    ...
                    parsed, parse_failures = parse_reasoned_output(
...
def main() -> None:
    ...
    parser.add_argument(
        "--products",
        type=Path,
        default=Path("data/interim/facet_discovery/i0030_products_clean_dedup.csv"),
    )
    parser.add_argument(
        "--sellers",
        type=Path,
        default=Path("data/processed/domeggook/seller_offers_core.csv"),
    )
    parser.add_argument(
        "--demands",
        type=Path,
        default=Path("data/synthetic/consumer_reference/grounded_demand_v2_1000.csv"),
    )
    parser.add_argument(
        "--boards",
        type=Path,
        default=Path(
            "data/synthetic/consumer_reference/demand_board_snapshot_5000.json"
        ),
    )
    parser.add_argument(
        "--queries",
    ...
    reason_columns = [
        ...
        "data_selection_reason",
    ]
    candidate_frame.reindex(columns=reason_columns, fill_value="").sort_values(
        ["model", "category_key", "name", "value"]
    ).to_csv(
        args.output_dir / "multisource_model_reason_comparison_v1.csv",
        index=False,
        encoding="utf-8-sig",
    )
    selected.to_csv(
        args.output_dir / "multisource_selected_candidates_v1.csv",
        index=False,
        encoding="utf-8-sig",
    )
    pd.DataFrame(failures).to_csv(
        args.output_dir / "multisource_model_failures_v1.csv",
        index=False,
        encoding="utf-8-sig",
    )
    source_counts = (
        data["source_type"].value_counts().to_dict() if not data.empty else {}
    )
    report = {
        "status": "COMPLETED",
        ...
        "category_row_counts": data.groupby(["category_key", "source_type"])
        .size()
        ...
        "consensus_selected_rows": int(
            selected.selection_status.eq("SELECTED_CANDIDATE").sum()
        )
        if not selected.empty
        else 0,
        "reason_present_rows": int(candidate_frame.reason_status.eq("PRESENT").sum())
        if not candidate_frame.empty and "reason_status" in candidate_frame
        else 0,
        "failure_rows": len(failures),
        "excluded_sources": ["GROUNDED_DEMAND_SYNTHETIC", "DEMAND_BOARD_SYNTHETIC"],
        "taxonomy_changed": False,
        "sampling": {
            "products_per_category": args.max_products_per_category,
            "sellers_per_category": args.max_sellers_per_category,
            "queries_per_category": args.max_queries_per_category,
        },
        "price_used_as_facet_evidence": False,
    }
    (args.output_dir / "multisource_facet_discovery_report_v1.json").write_text(
        json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    lines = [
        "# 다중 소스 Facet Discovery V1",
        "",
        f"- 대상 Category: {len(categories)}개",
        f"- 입력 행: {len(data):,}건",
        f"- 전체 후보: {len(candidate_frame):,}건",
        f"- 합의 후보: {report['consensus_selected_rows']:,}건",
        f"- 이유 포함 후보: {report['reason_present_rows']:,}건",
        f"- 실패: {len(failures):,}건",
        "",
        "## 입력 소스",
        "",
    ...
    lines += [
        ...
        "## 모델별 실행",
        "",
        "| 모델 | 호출 | 배치 | 후보 | 실패 | 실행 시간(초) |",
        "|---|---:|---:|---:|---:|---:|",
    ]
    lines.extend(
        f"| {item['model']} | {item['calls']} | {item.get('batches', 0)} | {item['candidate_rows']} | {item['failure_rows']} | {item['runtime_seconds']} |"
    ...
    lines += [
        ...
        "## 모델별 선정 이유",
        "",
        "아래 이유는 모델별 원본 후보 설명입니다. 모델 간 공통 후보가 아니어도 각 모델의 판단을 비교할 수 있습니다.",
    ]
    for model_name, group in (
        candidate_frame.groupby("model", sort=True)
        if "model" in candidate_frame.columns
        else []
    ):
        lines += [
            "",
            f"### {model_name}",
            "",
            "| Category | Facet | Value | 실제 데이터 관찰 요약 | 모델 설명 | 값의 의미 |",
            "|---|---|---|---|---|---|",
        ]
        for row in group.head(20).itertuples():
            lines.append(
                f"| {row.category_key} | {row.name} | {row.value} | {row.data_selection_reason} | {row.selection_reason} | {row.value_reason} |"
            )
    lines += [
        "",
        "## 해석",
        "",
        "data_selection_reason은 모델별 후보가 실제 입력 Category에서 몇 건 관찰됐고 어떤 출처 유형에 분포하는지 계산한 설명입니다. selection_reason과 value_reason은 모델이 반환한 보조 설명입니다.",
        "",
        "합성 구매 요청과 수요 보드의 가격·시간·참여자 수는 실제 사용자 행동으로 해석하지 않습니다.",
    ]
