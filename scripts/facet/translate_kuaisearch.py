...
NORMALIZATION_RULES = {
    ...
    "钙片": (
        "칼슘정",
        ("칼슘 정", "칼슘제제", "칼슘 제제", "칼슘 제품", "칼슘 보조제", "钙片"),
    ),
    "软糖": (
        "젤리",
        ("소프트 글루", "소프트캔디", "소프트 캔디", "소프트 케이크", "软糖"),
    ),
}
...
def numeric_tokens(value: str) -> set[str]:
    ...
    translated = str(value).translate(
        str.maketrans("零〇一二三四五六七八九", "00123456789")
    )
    tokens = set()
...
def _translate_batch(
    rows: list[dict[str, str]], model: str, endpoint: str, timeout: int
) -> list[dict[str, str]]:
        ...
        return [
            {
                ...
                "query_translated": payload["message"]["content"],
            }
        ]
    hints = {
        word: values[0]
        for word, values in GLOSSARY.items()
        if any(word in row["query_raw"] for row in rows)
    }
    prompt = (
            ...
            "translations": {
                ...
                "items": {
                    ...
                    "properties": {
                        "source_record_id": {
                            "type": "string",
                            "enum": [row["source_record_id"] for row in rows],
                        },
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Translate local KuaiSearch health queries into Korean"
    )
    parser.add_argument(
        "--input",
        type=Path,
        default=Path("data/interim/facet_evidence/kuaiseach_health_queries.parquet"),
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("data/interim/facet_evidence/kuaiseach_health_queries_ko.parquet"),
    )
    ...
    previous = (
        pd.read_parquet(args.output).fillna("")
        if args.output.exists()
        else pd.DataFrame()
    )
    cache = (
        dict(zip(previous.get("query_raw", []), previous.get("query_translated", [])))
    ...
    for start in range(0, len(pending_indices), args.batch_size):
        ...
        if not pending.empty:
            ...
            try:
                ...
                for item in translated:
                    ...
                    raw_by_id = {
                        str(row.source_record_id): str(row.query_raw)
                        for row in pending.itertuples()
                    }
                ...
                for index, row in pending.iterrows():
                    if str(row.source_record_id) in lookup:
                        frame.at[index, "query_translated"] = lookup[
                            str(row.source_record_id)
                        ]
                if expected - lookup.keys():
                    raise ValueError(
                        "Missing response IDs: "
                        + ", ".join(sorted(expected - lookup.keys()))
                    )
            except Exception as exc:
                failures += 1
                errors.append(
                    {
                        "source_record_ids": pending.source_record_id.astype(
                            str
                        ).tolist(),
                        "error": str(exc),
                    }
        ...
        print(
            {
                "translated": int(frame.query_translated.ne("").sum()),
                "rows": len(frame),
                "calls": calls,
                "failures": failures,
            },
    ...
    frame.to_parquet(args.output, index=False)
    frame.to_csv(
        args.output.with_name(args.output.stem + "_preview.csv"),
        index=False,
        encoding="utf-8-sig",
    )
    remaining = int(frame.query_translated.eq("").sum())
    report = {
        "status": "COMPLETED" if remaining == 0 else "COMPLETED_WITH_WARNINGS",
        "rows": len(frame),
        "translated": len(frame) - remaining,
        "remaining": remaining,
        "calls": calls,
        "failures": failures,
