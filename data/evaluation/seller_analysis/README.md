...
| 항목 | 값 |
|---|---|
| 평가 데이터셋 버전 | `seller_analysis_eval_v1` |
| 계산 정책 버전 | `seller-metrics-v1` |
| 응답 비율 필드 | 소수점 이하 최대 4자리(`RATIO_PRECISION=4`) |
| 사람이 읽는 근거 문장 | 소수점 이하 2자리(`DISPLAY_PRECISION=2`) |
| `supply_coverage_ratio` 상한 | `1.0` — 공급 충분 여부는 표시값이 아니라 원본 정수 비교로 판정 |

평가셋의 정답은 계산 정책 버전 `seller-metrics-v1`을 기준으로 해석한다. 정책 버전이
변경되면 기존 평가셋을 조용히 덮어쓰지 않고 새 버전의 평가셋과 README를 함께 생성한다.
...
`supply_coverage_ratio`는 `maximum_supply_quantity / total_demand_quantity`를
계산한 뒤 응답 지표에서는 `1.0`으로 상한을 적용한다. 공급 충분 여부는 표시용
비율이 아니라 원본 정수 비교로 판정하고, 상한을 적용한 경우 원본 비율을
`calculation_evidence`에 남긴다. 따라서 이 평가셋의 공급 충족률 기대값은
현재 정책과 일치한다. 기존 문서의 `1.15`·`1.25` 예시는 상한 적용 전 원본
계산값 또는 MOQ 예시로 해석하며, 정책 버전이 통일될 때까지 그 충돌을 보류
사항으로 기록한다.
...
| # | 항목 | 지금 어떻게 했나 |
|---|---|---|
| 1 | **거절 Case 의 `expected_request_result` 값** | `REJECTED` 로 적었다. 16절의 예시는 정상 Case 의 `SUCCESS` 만 보여 준다. 거절 쪽 값의 표기를 확인해야 한다 |
| 2 | **「수요량 매우 적음 / 매우 큼」 의 수치 기준** | 각각 총수요 10 이하 · 10,000 이상으로 잡았다. 문서에 기준이 없다 |
| 3 | **비율의 자리수와 반올림 방식** | 현재 구현 정책은 응답 수치 최대 소수 4자리, 근거 문장 소수 2자리다. 평가셋은 경계가 흔들리지 않도록 소수 4자리에서 정확히 떨어지는 입력으로 구성했다 |
| 4 | **Field 명** | 16절은 *"실제 API Response의 최종 Field명은 Backend / AI API Contract와 동일하게 맞춘다"* 고 한다. 현재 응답은 `metrics` 중첩이고 이 평가셋은 25절의 평면 Field 명을 쓴다. 평가 실행기가 대응시킨다 |
