...
@pytest.fixture(scope="module", params=["base", "overlays/dev"])
def a_labeling_resources(request, tmp_path_factory):
    kubectl = shutil.which("kubectl")
    if kubectl is None:
        pytest.skip("kubectl is required for offline Kustomize checks")
    environment = dict(os.environ)
...
def test_a_labeling_is_suspended_and_writes_through_database(a_labeling_resources):
    cronjob = a_labeling_resources["CronJob"]
...
def test_a_labeling_secret_and_http_contract(a_labeling_resources):
    cronjob = a_labeling_resources["CronJob"]
    pod = cronjob["spec"]["jobTemplate"]["spec"]["template"]["spec"]
    container = pod["containers"][0]
    assert container["env"] == [{
