...
def test_aihub_category_hierarchy_preserves_parent_path(tmp_path):
    frame = pd.DataFrame(
        {
            "source_category_path": ["01_가공식품 > 15_건강식품"],
            "kan_code": ["01150101"],
        }
    )
...
def test_category_path_ignores_empty_levels():
    assert (
        category_path({"category1": "식품", "category2": "", "category3": "건강"})
        == "식품 > 건강"
    )
...
def test_taxonomy_is_deterministic_and_all_is_zero():
    terms = pd.DataFrame(
        [{"term": "분말", "document_count": 3}, {"term": "캡슐", "document_count": 2}]
    )
